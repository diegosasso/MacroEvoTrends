...
p1 <- plot_enrichment_entropy(tree, enr[,1:2, drop=F], type = "Neff_scaled", n_points = 1000)
p2 <- plot_enrichment_entropy(tree, enr[,3:8, drop=F], type = "Neff_scaled", n_points = 1000)
p3 <- plot_enrichment_entropy(tree, enr[,9:10, drop=F], type = "Neff_scaled", n_points = 1000)
p4 <- plot_enrichment_entropy(tree, enr[,11:13, drop=F], type = "Neff_scaled", n_points = 1000)
p5 <- plot_enrichment_entropy(tree, enr[,14:15, drop=F], type = "Neff_scaled", n_points = 1000)
#p1$plot
#head(p1$data)
```
...
# Add group names
p1$data$group <- "head"
p2$data$group <- "mesosoma"
p3$data$group <- "metasoma"
p4$data$group <- "legs"
p5$data$group <- "wings"

# Combine
df_all <- bind_rows(p1$data, p2$data, p3$data, p4$data, p5$data)
# Fix facet order
...
# Plot vertically stacked subplots
ggplot(df_all, aes(x = time, y = entropy)) +
  geom_area(fill = "#33a02c", alpha = 0.6) +
  #geom_line(color = "#006d2c", size = .5) +
  geom_hline(yintercept = 0, color = "#33a02c", linetype = "solid", size = 0.7, alpha = 0.6) +  # <-- here
  scale_x_reverse(
    breaks = seq(0, max(df_all$time), by = 50),
    expand = c(0, 0)
  ) +
  scale_y_continuous(
    #expand = c(0, 0)
  ...
  labs(y = "Neff (scaled)", x = "Time (Ma)")

ggsave("plots/Neff-BRs.pdf", width = 10, height = 5, units = "in")
```

## Strength
```{r}
s1 <- plot_enrichment_strength(tree, enr[,1:2, drop=F],   type = "not_scaled", n_points = 1000)
s2 <- plot_enrichment_strength(tree, enr[,3:8, drop=F],   type = "not_scaled", n_points = 1000)
s3 <- plot_enrichment_strength(tree, enr[,9:10, drop=F],  type = "not_scaled", n_points = 1000)
s4 <- plot_enrichment_strength(tree, enr[,11:13, drop=F], type = "not_scaled", n_points = 1000)
s5 <- plot_enrichment_strength(tree, enr[,14:15, drop=F], type = "not_scaled", n_points = 1000)
# s1$data
```
...
# Strength
s1$data$group <- "head"
s2$data$group <- "mesosoma"
s3$data$group <- "metasoma"
s4$data$group <- "legs"
s5$data$group <- "wings"

df_entropy <- bind_rows(p1$data, p2$data, p3$data, p4$data, p5$data) %>%
  mutate(type = "neff")

df_strength <- bind_rows(s1$data, s2$data, s3$data, s4$data, s5$data) %>%
  rename(entropy = strength) %>%
  mutate(type = "strength")

df_all <- bind_rows(df_entropy, df_strength) %>%
  mutate(group = factor(group, levels = c("head", "mesosoma", "metasoma", "legs", "wings")))


# --- Plot
ggplot() +
    geom_area(data = filter(df_all, type == "strength"),
            aes(x = time, y = entropy),
            fill = "#e31a1c", alpha = .2) +

  # Strength (red line)
  geom_line(data = filter(df_all, type == "strength"),
            aes(x = time, y = entropy),
            color = "#e31a1c", size = .2, , alpha = 1) +
  
  # Neff (green fill area)
    geom_area(data = filter(df_all, type == "neff"),
            aes(x = time, y = entropy),
            fill = 'white', alpha = 1) +
  ...
  geom_line(data = filter(df_all, type == "neff"),
            aes(x = time, y = entropy),
            color = "#006d2c", size = .2, linetype = 1, alpha = 1) +
              
  geom_hline(yintercept = 0, color = "#006d2c",
             linetype = "dashed", size = 0.7, alpha = 1) +
  ...
  labs(y = "Neff (green, scaled) / Strength (red)", x = "Time (Ma)")

ggsave("plots/Neff-S-BRs.pdf", width = 10, height = 5, units = "in")
```
...
# Rolling corr vs. metric

## read in df_avg

```{r}
# !!! remeber to updated df_avg !!!
df_avg <- readRDS('data/df_avg.rds')
head(df_avg)
```

```{r}
# First, make a scaled version of mean_cor to match the left y-axis
df_avg$mean_cor_scaled <- df_avg$mean_cor * max(df_all$metric_scaled, na.rm = TRUE)

# Plot
p <- ggplot() +
  # --- df_all metrics (Neff + Strength) ---
  geom_line(data = df_all, aes(x = time, y = metric_scaled, color = metric), size = 1) +
  geom_area(data = df_all, aes(x = time, y = metric_scaled, fill = metric),
            alpha = 0.2, position = "identity") +
  
  # --- Overlay df_avg (mean correlation) ---
  geom_line(data = df_avg, aes(x = time, y = mean_cor_scaled),
  ...
  scale_y_continuous(
    name = "Neff_scaled",
    expand = c(0, 0),
    sec.axis = sec_axis(~ . / max(df_all$metric_scaled, na.rm = TRUE),
                        name = "Mean correlation (black dashed)")
  ) +
