---
title: "Underrepresentation Plots"
date: "`r Sys.Date()`"
output: 
  html_document:
    df_print: paged
...
under_class <- under_class[c("head", "mesosoma", "metasoma", "legs",  "wings", "sclerites", "muscles")]
head(under_class)
head(under)
...
# Tip underrepresentation

```{r}

enr.masked <- get_enrichment_mask(rates_norm, lower_q = 0.05, upper_q = 0.95, direction = "under", return_type = "masked")
...
file_base <- "plots/heat_map-under-legend"

postscript(
  file = paste0(file_base, ".eps"),
  width = 15,          # width in inches (adjust for column width)
...
# ---- Save to EPS ----
if (!dir.exists("plots")) dir.create("plots")
file_base <- "plots/tree_enrich-under"
...
# ==========================
branch_colors <- ifelse(under.boolean == 1, "#1f78b4", "black")
branch_widths <- ifelse(under.boolean == 1, 6, 2)  # thicker if enriched
...
for (i in seq_along(under.enr)) {
  ...
  for (j in seq_len(n)) {
    ...
    rect(xleft, ybottom, xright, ytop,
         col = ifelse(bits[j] == 1, "#a6cee3" , "white"),
